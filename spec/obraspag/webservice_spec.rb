...
describe Braspag::Webservice do
  let(:webservice)  { Braspag::Webservice.new(:homolog) }
  ...
  context "should call authorize_request" do
    it "should call autorize_transaction with success" do
      pending
      webservice.authorize_transaction(authorize_request).should be_true
    end
  ...
  context "on .checkout" do
      ...
      it "should not call capture when authorize returns success false" do
        webservice.stub(:authorize_transaction).and_return(authorize_response_failure)
        webservice.checkout(authorize_request).should_not_receive(:capture_credit_card_transaction)
     end

      it "should call capture when authorize returns success true" do
        webservice.stub(:authorize_transaction).and_return(authorize_response_success)
        webservice.stub(:capture_credit_card_transaction).and_return(capture_response_success)
        webservice.checkout(authorize_request).should eq({:authorize_response => authorize_response_success, :capture_response => capture_response_success})
     end

     it "should call capture with the correct request data" do
      credit_card_request = Braspag::CreditCardTransactionRequest.new
      webservice.stub(:authorize_transaction).and_return(authorize_response_success)
      webservice.stub(:create_capture_credit_card_request).and_return(credit_card_request)
      webservice.should_receive(:capture_credit_card_transaction).with(credit_card_request).and_return(capture_response_success)
      webservice.checkout(authorize_request).should eq({:authorize_response => authorize_response_success, :capture_response => capture_response_success})
     end
  ...
  context "authorize response validation" do
    success_authorize_not_captured_response = {:authorize_transaction_response=>{:authorize_transaction_result=>{:correlation_id=>"00000000-0000-0000-0000-000000000007", :success=>true, :error_report_data_collection=>nil, :order_data=>{:order_id=>"45454545454545", :braspag_order_id=>"9bbdb10e-6566-4b34-9097-d595b67ce31d"}, :payment_data_collection=>{:payment_data_response=>{:braspag_transaction_id=>"a744d7c2-754a-4b26-94dd-e7f1435976ca", :payment_method=>"997", :amount=>"500", :acquirer_transaction_id=>"1031064345046", :authorization_code=>"20121031064345046", :return_code=>"4", :return_message=>"Operation Successful", :status=>"1", :credit_card_token=>nil, :"@xsi:type"=>"CreditCardDataResponse"}}}, :@xmlns=>"https://www.pagador.com.br/webservice/pagador"}}
    success_authorize_captured_response = {:authorize_transaction_response=>{:authorize_transaction_result=>{:correlation_id=>"00000000-0000-0000-0000-000000000007", :success=>true, :error_report_data_collection=>nil, :order_data=>{:order_id=>"45454545454545", :braspag_order_id=>"9bbdb10e-6566-4b34-9097-d595b67ce31d"}, :payment_data_collection=>{:payment_data_response=>{:braspag_transaction_id=>"a744d7c2-754a-4b26-94dd-e7f1435976ca", :payment_method=>"997", :amount=>"500", :acquirer_transaction_id=>"1031064345046", :authorization_code=>"20121031064345046", :return_code=>"4", :return_message=>"Operation Successful", :status=>"0", :credit_card_token=>nil, :"@xsi:type"=>"CreditCardDataResponse"}}}, :@xmlns=>"https://www.pagador.com.br/webservice/pagador"}}
    failure_authorize_response = {:authorize_transaction_response=>{:authorize_transaction_result=>{:correlation_id=>"00000000-0000-0000-0000-000000000000", :success=>false, :error_report_data_collection=>{:error_report_data_response=>[{:error_code=>"100", :error_message=>"RequestId is a mandatory parameter"}, {:error_code=>"107", :error_message=>"PaymentDataCollection is a mandatory item"}]}}, :@xmlns=>"https://www.pagador.com.br/webservice/pagador"}}

    it "should be success when succes field is true" do
      webservice.send(:successful_authorization?, success_authorize_not_captured_response).should eq(true)
    end

    it "should not be success when succes field is false" do
      webservice.send(:successful_authorization?, failure_authorize_response).should eq(false)
    end

    it "should send capture when status is 1" do
      webservice.send(:authorization_not_captured?, success_authorize_not_captured_response).should eq(true)
    end

    it "should send capture when status is <> 1" do
      webservice.send(:authorization_not_captured?, success_authorize_captured_response).should eq(false)
    end
